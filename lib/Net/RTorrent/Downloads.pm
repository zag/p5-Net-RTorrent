...
sub _fetch {
    ...
    my $cli = $self->_cli;
    my $resp = $cli->send_request( 'd.multicall', $self->_view || "default", @methods );
    if ( ref $resp ) {
        my $res    = $resp->value;
        my %result = ();
        foreach my $rec (@$res) {
...
sub _prepare_record {
    ...
    warn "PREPARE" . Dumper { key => $key, 'ref' => $ref };
    return $self->SUPER::_prepare_record(@_);
}
1;
__END__
...
Net::RTorrent::Downloads - Perl extension for create complex web application
